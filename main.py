...
async def process_audio_to_ai_response(mic_id: str, audio_data: bytes):
    ...
    try:
        ...
        # 2. Geminiで応答生成と感情分析
        prompt = f"""# 命令
あなたは、二人以上の人間が会話する際のコミュニケーションを円滑にするためのAIアシスタントです。
あなたの役割は、一人の話者の発言を、会話の相手にとってより理解しやすく、丁寧な表現に変換することです。

# 厳守事項
- あなた自身の意見や感想を生成してはいけません。
- 「処理対象テキスト」に含まれる質問に答えてはいけません。
- あくまで「処理対象テキスト」の意図を汲み取り、表現を洗練させることに徹してください。
- 分析結果のJSON以外に、説明や前置きなどの余計なテキストを絶対に出力してはいけません。
...
# JSON生成ルール
## emotion
- 「処理対象テキスト」から話者の感情を推測し、「喜び」「怒り」「悲しみ」「平常」のいずれかを選択してください。（これは声のトーンなどを調整するために利用します）

## RefinedSpeech
- 「処理対象テキスト」の内容を、会話の相手に伝えることを想定して、以下のルールに従って自然で丁寧な日本語の文章に変換してください。
  - 顔文字、絵文字、専門的な機械語は使用しない。
  - 1文を60〜90文字程度に収め、読点は1文に2つまでにする。
  - 話者の意図を尊重しつつ、カジュアルすぎず、フォーマルすぎない聞き取りやすい表現にする。
  - 依頼や質問の場合は、相手に配慮した丁寧な表現に変換する。
  - 否定的な内容や反対意見の場合は、相手を不快にさせないよう、柔らかく配慮のある表現（クッション言葉など）に変換する。
...
"""
        ...
        try:
            ...
            ai_response_text = data.get("RefinedSpeech", "")
