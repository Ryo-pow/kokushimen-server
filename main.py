...
async def process_audio_to_ai_response(mic_id: str, audio_data: bytes):
    ...
    try:
        ...
        # 2. Geminiで応答生成と感情分析
        prompt = f"""# 命令
あなたは、入力された日本語テキストを指定されたJSON形式に変換するAIプログラムです。
以下の指示を厳格に守り、処理を実行してください。

# 厳守事項 (最優先)
1.  **会話の禁止**: あなた自身の意見、感想、相槌、応答を絶対に生成してはいけません。
2.  **質問への回答禁止**: 「処理対象テキスト」が質問であっても、それに答えてはいけません。
3.  **出力形式の厳守**: 生成する応答は、後述する「出力形式」のJSONオブジェクトのみでなければなりません。説明、前置き、```json のようなMarkdownコードブロックは一切含めてはいけません。

# 処理対象テキスト
{transcribed_text}

# 実行タスク
「処理対象テキスト」を分析し、以下のルールに従ってJSONオブジェクトを生成してください。
...
## RefinedSpeech
- 「処理対象テキスト」の意図と意味を完全に保持したまま、会話の相手に伝えることを想定し、以下のルールに従って表現を「洗練」してください。
  - 顔文字、絵文字、専門的な機械語は使用しない。
  - 1文を60〜90文字程度に収め、読点は1文に2つまでにする。
  - カジュアルすぎず、フォーマルすぎない聞き取りやすい表現にする。
  - 依頼や質問の場合は、相手に配慮した丁寧な表現に変換する。
  - 否定的な内容や反対意見の場合は、クッション言葉などを用い、柔らかく配慮のある表現に変換する。
